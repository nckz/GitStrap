...
.non-active-nav { \
    ...
    filter: opacity(75%); \
} ';window.onload=function(){if(document.head==null){alert("You must update your browser to view this website.");}
document.head.innerHTML=gs_html_head_tag;document.body.innerHTML=gs_html_body_tag;setTitle(gsConfig.title);setTheme(gsConfig.theme);add_style_tag(gs_html_style_tag);var scripts=[gs_jquery_url,gs_bootstrap_url,gs_showdown_url,gs_jsyaml_url];loadAndExecuteScripts(scripts,0,gs_jq_start);}
...
function renderPage(){var gsNav=new Nav(gsConfig);if(gsConfig.headerIsActive()&&!gsConfig.postIsActive()){MarkdownToHTML(gsConfig.header,gs_header_id);}
