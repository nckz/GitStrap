...
var gs_jquery_url="https://ajax.googleapis.com/ajax/libs/jquery/1.12.0/jquery.min.js";var gs_bootstrap_url="http://maxcdn.bootstrapcdn.com/bootstrap/3.3.6/js/bootstrap.min.js";var gs_showdown_url="https://cdn.rawgit.com/showdownjs/showdown/1.3.0/dist/showdown.min.js";var gs_jsyaml_url="https://cdn.rawgit.com/nckz/GitStrap/fa12906d40aac6596f8d7601fddf7b21dc9b47a3/js/js-yaml-front-client.min.js"
...
var gs_bootgitstrap_url="https://cdn.rawgit.com/nckz/GitStrap/gh-pages/js/bootgitstrap.min.js";var gs_css_url="https://cdn.rawgit.com/nckz/GitStrap/gh-pages/css/gitstrap.css"
...
document.head.innerHTML=gs_html_head_tag;document.body.innerHTML=gs_html_body_tag;add_style(gs_default_theme_url);add_style(gs_css_url);var scripts=[gs_jquery_url,gs_bootstrap_url,gs_showdown_url,gs_jsyaml_url,gs_bootgitstrap_url];loadAndExecuteScripts(scripts,0,function(){});}
...
function PostToHTML(relpath,markdown_body){var callback=function(text){text=text.replace("(((",'---');text=text.replace(")))",'---');console.log(text);function isBlank(str){return(!str||/^\s*$/.test(str));}
var body=document.getElementById(markdown_body);var post_head=document.createElement('div');var post_body=document.createElement('div');body.appendChild(post_head);body.appendChild(post_body);if(isBlank(text)){post_head.innerHTML='Unable to load text for: '+relpath;return;}
