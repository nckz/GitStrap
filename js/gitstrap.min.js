...
function PostToHTML(relpath,markdown_body){var callback=function(text){var body=document.getElementById(markdown_body);var post_head=document.createElement('div');var post_body=document.createElement('div');body.appendChild(post_head);body.appendChild(post_body);var obj=jsyaml.loadFront(text)
post_head.innerHTML='<b>'+obj.Title+'</b><br> &nbsp; &nbsp; '+
obj.Author+' | <small>'+obj.Date.toDateString()+'</small><br><br>';var converter=new showdown.Converter();post_body.innerHTML=converter.makeHtml(obj.__content);};getFile(relpath,callback);}
